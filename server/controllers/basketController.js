    ...
    async get(req, res, next) {
        try {
            ...
            if (user.id === basket.userId) {
                ...
                next(ApiError.forbidden(`Не свою коляску катишь, ♿️`))
            )
        } catch (error) {
    ...
    async addToBasket(req, res, next) {
        try {
            ...
            if (user.id === basket.userId) {
                ...
                return res.status(201).json({ message: `Создана связь товар-корзина: Корзина — ${id}, Товар — ${productId}` })
            }
            else (
                next(ApiError.forbidden(`Не свою коляску катишь, ♿️`))
            )
    ...
    async removeFromBasket(req, res, next) {
        try {
            ...
            if (user.id === basket.userId) {
                ...
                const basket_product = await BasketProduct.findOne({ where: {basketId: basket.id}})
                ...
                if (basket_product.productId === product.id) {
                    ...
                    next(ApiError.badRequest(`Нет ♿️ для удаления из корзины`))
                }
            }
            else (
                next(ApiError.forbidden(`Не свою коляску катишь, ♿️`))
